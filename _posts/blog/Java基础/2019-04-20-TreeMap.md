...
``` java
// key检查，如果comparator为null，那么key是不能为null的，并且key是实现Comparable接口的，因为Tree默认是有序的，需要比较.
...
private void fixAfterInsertion(Entry<K,V> x) {
    ...
    while (x != null && x != root && x.parent.color == RED) {
        ...
        if (parentOf(x) == leftOf(parentOf(parentOf(x)))) {//如果父节点是左节点
            Entry<K,V> y = rightOf(parentOf(parentOf(x)));//获取当前节点的叔叔节点，也就是他父节点的兄弟节点，这个也可能为null
    ...
    while (x != root && colorOf(x) == BLACK) {
        ...
        if (x == leftOf(parentOf(x))) {//x是左节点
            ...
            if (colorOf(leftOf(sib))  == BLACK &&
                ...
                // 最后是对root置为黑色，基本没有影响（因为root节点本来就是黑色）,这里的代码让sib的颜色等于x父节点的颜色，基本没影响，
                // 其实他最终目的是让x所在分支增加一个黑色节点，来达到红黑平衡。
