    ...
    private void deleteEntry(Entry<K,V> p) {
        ...
        if (replacement != null) {
            ...
            if (p.parent == null)//如果p的父节点为null，说明p是root节点，所以replacement是p唯一的节点，把p节点删除后，让
                //replacement成为root节点
                root = replacement;
            else if (p == p.parent.left)//这个不会变，原来p是左节点就让replacement成为左节点，原来p为右节点就让replacement成为右节点。相当于替换p节点的位置
        ...
        while (x != root && colorOf(x) == BLACK) {
            ...
            if (x == leftOf(parentOf(x))) {//x是左节点
                ...
                if (colorOf(leftOf(sib))  == BLACK &&
                    ...
                    // .执行到这一步，从root到sib分支的黑色节点是没有变化，但从root到x分支的黑色节点是少了一个，然后执行下面的代码会使x的兄弟分支黑色节点不变
